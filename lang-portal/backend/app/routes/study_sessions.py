...
def load(app):
  ...
  def create_study_session():
    try:
      ...
      required_fields = ['user_id', 'word_ids', 'session_type']
      if not all(field in data for field in required_fields):
        return jsonify({"error": "Missing required fields"}), 400
      
      if not isinstance(data['word_ids'], list):
        return jsonify({"error": "word_ids must be an array"}), 400

      cursor = app.db.cursor()
      
      # Insert study session
      cursor.execute('''
          INSERT INTO study_sessions (user_id, session_type, created_at)
          VALUES (?, ?, CURRENT_TIMESTAMP)
      ''', (data['user_id'], data['session_type']))
      
      session_id = cursor.lastrowid
      
      # Insert word associations
